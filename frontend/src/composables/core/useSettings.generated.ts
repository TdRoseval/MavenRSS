 ...
 * 
 * IMPORTANT: This function preserves empty values from the backend instead of replacing them with defaults.
 * This prevents the auto-save mechanism from overwriting user's custom settings with default values.
 * Default values are only used for display purposes in the UI, not for data storage.
 */
export function parseSettingsData(data: Record<string, string>): SettingsData {
  return {
    ai_api_key: data.ai_api_key ?? '',
    ai_chat_enabled: data.ai_chat_enabled === 'true',
    ai_chat_profile_id: data.ai_chat_profile_id ?? '',
    ai_custom_headers: data.ai_custom_headers ?? '',
    ai_endpoint: data.ai_endpoint ?? '',
    ai_model: data.ai_model ?? '',
    ai_search_enabled: data.ai_search_enabled === 'true',
    ai_search_profile_id: data.ai_search_profile_id ?? '',
    ai_summary_profile_id: data.ai_summary_profile_id ?? '',
    ai_summary_prompt: data.ai_summary_prompt ?? '',
    ai_translation_profile_id: data.ai_translation_profile_id ?? '',
    ai_translation_prompt: data.ai_translation_prompt ?? '',
    ai_usage_hard_limit: data.ai_usage_hard_limit ?? '',
    ai_usage_limit: data.ai_usage_limit ?? '',
    ai_usage_tokens: data.ai_usage_tokens ?? '',
    ...
    content_font_size: parseInt(data.content_font_size) || 0,
    content_line_height: data.content_line_height ?? '',
    custom_css_file: data.custom_css_file ?? '',
    custom_translation_body_template: data.custom_translation_body_template ?? '',
    custom_translation_enabled: data.custom_translation_enabled === 'true',
    custom_translation_endpoint: data.custom_translation_endpoint ?? '',
    custom_translation_headers: data.custom_translation_headers ?? '',
    custom_translation_lang_mapping: data.custom_translation_lang_mapping ?? '',
    custom_translation_method: data.custom_translation_method ?? '',
    custom_translation_name: data.custom_translation_name ?? '',
    custom_translation_response_path: data.custom_translation_response_path ?? '',
    custom_translation_timeout: parseInt(data.custom_translation_timeout) || 0,
    deepl_api_key: data.deepl_api_key ?? '',
    deepl_endpoint: data.deepl_endpoint ?? '',
    default_view_mode: data.default_view_mode ?? '',
    feed_drawer_expanded: data.feed_drawer_expanded === 'true',
    feed_drawer_pinned: data.feed_drawer_pinned === 'true',
    freshrss_api_password: data.freshrss_api_password ?? '',
    freshrss_auto_sync_interval: parseInt(data.freshrss_auto_sync_interval) || 0,
    freshrss_enabled: data.freshrss_enabled === 'true',
    freshrss_last_sync_time: data.freshrss_last_sync_time ?? '',
    freshrss_server_url: data.freshrss_server_url ?? '',
    freshrss_sync_on_startup: data.freshrss_sync_on_startup === 'true',
    freshrss_username: data.freshrss_username ?? '',
    full_text_fetch_enabled: data.full_text_fetch_enabled === 'true',
    google_translate_endpoint: data.google_translate_endpoint ?? '',
    hover_mark_as_read: data.hover_mark_as_read === 'true',
    idle_conn_timeout_seconds: parseInt(data.idle_conn_timeout_seconds) || 0,
    image_gallery_enabled: data.image_gallery_enabled === 'true',
    language: data.language ?? '',
    last_global_refresh: data.last_global_refresh ?? '',
    last_network_test: data.last_network_test ?? '',
    layout_mode: data.layout_mode ?? '',
    max_article_age_days: parseInt(data.max_article_age_days) || 0,
    max_cache_size_mb: parseInt(data.max_cache_size_mb) || 0,
    max_concurrent_refreshes: data.max_concurrent_refreshes ?? '',
    max_conns_per_host: parseInt(data.max_conns_per_host) || 0,
    max_idle_conns: parseInt(data.max_idle_conns) || 0,
    max_idle_conns_per_host: parseInt(data.max_idle_conns_per_host) || 0,
    media_cache_enabled: data.media_cache_enabled === 'true',
    media_cache_max_age_days: parseInt(data.media_cache_max_age_days) || 0,
    media_cache_max_size_mb: parseInt(data.media_cache_max_size_mb) || 0,
    ...
    retry_timeout_seconds: parseInt(data.retry_timeout_seconds) || 0,
    rsshub_api_key: data.rsshub_api_key ?? '',
    rsshub_enabled: data.rsshub_enabled === 'true',
    rsshub_endpoint: data.rsshub_endpoint ?? '',
    rules: data.rules ?? '',
    ...
    summary_trigger_mode: data.summary_trigger_mode ?? '',
    ...
    update_interval: parseInt(data.update_interval) || 0,
    window_height: data.window_height ?? '',
    window_maximized: data.window_maximized ?? '',
