...
const feedId = computed(() => articleStore.currentFeedId);

// Compute which category to fetch (if viewing a specific category)
const category = computed(() => articleStore.currentCategory);
...
function openArticleDetail(): void {
  ...
  articleStore.currentArticleId = selectedArticle.value.id;
