...
import ArticleFilterModal from '@/components/modals/filter/ArticleFilterModal.vue';
...
import { useArticleTranslation } from '@/features/article/composables/useArticleTranslation';
import { useArticleFilter } from '@/features/article/composables/useArticleFilter';
import { useArticleActions } from '@/features/article/composables/useArticleActions';
...
import { proxyImagesInHtml, isMediaCacheEnabled } from '@/shared/lib/mediaProxy';
...
const filteredArticlesFromServer = computed(() => articleStore.filteredArticlesFromServer);
const isFilterLoading = computed(() => articleStore.isFilterLoading);

// Computed filtered articles - optimized to avoid excessive recomputation
const filteredArticles = computed(() => {
  // If AI search is active, use AI search results
  if (isAISearchActive.value && articleStore.aiSearchResults.length > 0) {
  ...
  if (articleStore.showOnlyUnread && temporarilyKeepArticles.value.size > 0) {
    articles = articles.filter(
      (article) => !article.is_read || temporarilyKeepArticles.value.has(article.id)
    );
  } else if (articleStore.showOnlyUnread) {
...
function handleAISearchResults(articles: Article[]) {
  articleStore.setAISearchResults(articles);
}

function handleAISearchClear() {
  articleStore.clearAISearchResults();
}
...
const articleListTitle = computed(() => {
  ...
  if (articleStore.tempSelection.feedId) {
    const feed = feedStore.feeds?.find((f) => f.id === articleStore.tempSelection.feedId);
  ...
  if (articleStore.tempSelection.category) {
    const categoryName = articleStore.tempSelection.category;
    const filterText = getFilterText();
...
async function loadArticleListSettings() {
  ...
  try {
    ...
    if (translationSettings.value.enabled && listRef.value) {
      setupIntersectionObserver(listRef.value, articleStore.articles);
    }
  } catch (e) {
...
function selectArticle(article: Article): void {
  ...
  const feed = feedStore.feeds.find((f) => f.id === article.feed_id);
  ...
  if (articleStore.currentArticleId) {
    temporarilyKeepArticles.value.delete(articleStore.currentArticleId);
  }
...
async function handleApplyFilters(filters: typeof activeFilters.value): Promise<void> {
  ...
  if (filters.length === 0) {
    ...
    await articleStore.fetchArticles(false);
  } else {
...
async function markAllAsRead(): Promise<void> {
  ...
  if (activeFilters.value.length > 0) {
    ...
    } else if (articleStore.currentCategory) {
      params.category = articleStore.currentCategory;
    }

    await articleStore.markAllAsRead(params.feed_id, params.category);
    window.showToast(t('article.action.markedAllAsRead'), 'success');
  }
}
...
<template>
  ...
  >
    <div class="p-2 sm:p-4 border-b border-border bg-bg-primary">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-1 sm:gap-2">
          ...
          <button
            v-if="articleStore.currentFilter === 'readLater'"
          ...
          <button
            class="text-text-secondary hover:text-text-primary hover:bg-bg-tertiary p-1 sm:p-1.5 rounded transition-colors"
            :class="articleStore.showOnlyUnread ? 'text-accent' : ''"
            :title="
              articleStore.showOnlyUnread
                ? t('setting.reading.showAllArticles')
                : t('setting.reading.showOnlyUnread')
            "
            @click="articleStore.toggleShowOnlyUnread()"
          >
            <component
              :is="articleStore.showOnlyUnread ? PhEyeSlash : PhEye"
              :size="18"
              class="sm:w-5 sm:h-5"
            />
          </button>
    ...
    <div ref="listRef" class="flex-1 overflow-y-scroll article-list-scroll" @scroll="handleScroll">
      ...
      <div
        v-if="isAISearchActive && filteredArticles.length === 0 && !articleStore.isLoading"
        class="p-4 sm:p-5 text-center text-text-secondary text-sm sm:text-base"
      >
      ...
      <div v-else>
        ...
        <div class="article-list-container">
          <ArticleItem
            ...
            :key="article.id"
            :article="article"
            :is-active="articleStore.currentArticleId === article.id"
            @click="selectArticle(article)"
            @contextmenu="(e) => showArticleContextMenu(e, article)"
      ...
      <div
        v-if="articleStore.isLoading || isFilterLoading"
