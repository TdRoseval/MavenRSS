...
export function useArticleDetail() {
  ...
  const article = computed<Article | undefined>(() => {
    ...
    return articles.find((a: Article) => a.id === articleStore.currentArticleId);
  });
  ...
  function getEffectiveArticlesList(): Article[] {
    ...
    if (articleStore.aiSearchResults.length > 0) {
    ...
    if (articleStore.activeFilters.length > 0) {
      return articleStore.filteredArticlesFromServer;
    }
  ...
  const currentArticleIndex = computed(() => {
    if (!articleStore.currentArticleId) return -1;
    const articles = getEffectiveArticlesList();
    return articles.findIndex((a: Article) => a.id === articleStore.currentArticleId);
  });
  ...
  function goToPreviousArticle() {
    ...
    articleStore.currentArticleId = prevArticle.id;
  ...
  function goToNextArticle() {
    ...
    articleStore.currentArticleId = nextArticle.id;
  ...
  function getEffectiveViewMode(): ViewMode {
    ...
    const feed = feedStore.feeds.find((f: Feed) => f.id === article.value!.feed_id);
    ...
    async (newId, oldId) => {
      if (newId && newId !== oldId) {
        ...
        if (pendingRenderAction.value) {
          ...
          const storedPreference = articleStore.articleViewModePreferences.get(newId);
  ...
  watch(
    () => [articleStore.currentFeedId, articleStore.currentFilter, articleStore.currentCategory],
    () => {
  ...
  window.addEventListener('default-view-mode-changed', (e: Event) => {
    ...
    articleStore.articleViewModePreferences.clear();
  });
  ...
  async function toggleContentView() {
    ...
    if (article.value) {
      ...
      articleStore.articleViewModePreferences.set(article.value.id, mode);
  ...
  async function fetchArticleContent() {
    ...
    currentArticleId.value = targetId; // Track which article we're loading
    ...
    try {
      ...
      if (proxyEnabled && content) {
        ...
        content = proxyMediaInHtml(content, feedUrl, authStore.accessToken || undefined, feedId);
      }
  ...
  async function refreshArticle() {
    ...
    try {
      ...
      await authPost(`/api/articles/refresh?id=${targetId}`);

      // Clear local cached content
      articleContent.value = '';
      currentArticleId.value = null;

      // Ensure we're still on the same article before modifying it
      if (article.value && article.value.id === targetId) {
  ...
  onMounted(async () => {
    ...
    if (articleStore.articleViewModePreferences.size === 0) {
      try {
        ...
        if (saved) {
          ...
          Object.entries(preferences).forEach(([articleId, mode]) => {
            articleStore.articleViewModePreferences.set(Number(articleId), mode);
          });
        }
      } catch (e) {
    ...
    if (articleStore.currentArticleId) {
      ...
      if (currentArticleId.value !== articleStore.currentArticleId || !articleContent.value) {
        await fetchArticleContent();
      }
    }
