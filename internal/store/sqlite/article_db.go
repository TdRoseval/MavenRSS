...
func (db *DB) SaveArticle(article *models.Article) error {
	...
	// For new articles, check two conditions:
	// 1. Original published time only has date (00:00:00)
...
func (db *DB) SaveArticles(ctx context.Context, articles []*models.Article) error {
	...
	// Step 1: Save original published time, and use it to generate unique ID
	...
	// Step 3: Core logic! For all new articles, check two conditions:
	// 1. Original published time only has date (00:00:00)
	// 2. Original published time is in the future
	now := time.Now().UTC()
	for i, article := range articles {
