...
func TestGetEncryptedSettingNotFound(t *testing.T) {
	...
	val, err := db.GetEncryptedSetting("non_existent_key_xyz")
	if err != nil {
		t.Errorf("GetEncryptedSetting() unexpected error for non-existent key: %v", err)
	}
	if val != "" {
		t.Errorf("GetEncryptedSetting() expected empty string for non-existent key, got %q", val)
	}
}
