...
仓库内已经包含 `docker-compose.yml` 和服务端 Dockerfile（`Dockerfile.server` 使用国际源，`DockerfileCN.server` 使用国内镜像加速，`docker compose` 默认走 `DockerfileCN.server`），本地体验 Web 版最直接的方式是：
...
> **提示**：生产环境请务必设置 `MRRSS_JWT_SECRET`、`MRRSS_ADMIN_PASSWORD`、`MRRSS_TEMPLATE_PASSWORD` 等关键变量；若留空，服务端会回退到不安全的内置默认值（如 `admin123`）。除 `MRRSS_DEBUG` 外，`docker-compose.yml` 中的变量默认均被注释掉，需自行取消注释并填写。
...
- [Node.js](https://nodejs.org/) LTS + npm（要求 `>= 20.19`，推荐 `20.19+` / `22.12+`）
